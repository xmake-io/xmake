...
function _check_from_arglist(flags, opt, islinker)
    ...
    -- make cache key
    local key = "detect.tools.ldc2.has_flags"
...
function _check_try_running(flags, opt, islinker)
    ...
    local sourcefile = path.join(os.tmpdir(), "detect", "ldc_has_flags.d")
    local objectfile = path.join(os.tmpdir(), "detect", "ldc_has_flags.o")

...
rule("qt.qrc")
    ...
    on_buildcmd_file(function (target, batchcmds, sourcefile_qrc, opt)
        ...
        batchcmds:vrunv(path(rcc), {"-name", path.basename(sourcefile_qrc), path(sourcefile_qrc), "-o", path(sourcefile_cpp)})
        batchcmds:compile(path(sourcefile_cpp), path(objectfile))

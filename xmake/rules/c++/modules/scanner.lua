...
function _patch_sourcebatch(target, sourcebatch)
    ...
    if not cached_sourcebatch then
        ...
        local cached_md5sum = localcache:get2(target:fullname(), "sourcebatch_md5sum")
        if not cached_md5sum or cached_md5sum ~= md5sum then
            localcache:set2(target:fullname(), "sourcebatch_md5sum", md5sum)

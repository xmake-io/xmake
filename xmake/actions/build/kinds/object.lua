...
function _build_from_object(target, sourcefile, objectfile, progress)
    ...
    -- trace progress info
    if verbose then
        cprint("${build.progress}[%3d%%]: ${dim magenta}inserting.$(mode) %s", progress, sourcefile)
    else
        cprint("${build.progress}[%3d%%]: ${magenta}inserting.$(mode) %s", progress, sourcefile)
    end
...
function _do_build_file(target, sourcefile, opt)
    ...
    -- trace progress info
    if verbose then
        cprint("${build.progress}[%3d%%]:${dim} %scompiling.$(mode) %s", progress, _g.ccache and "ccache " or "", sourcefile)
    else
        cprint("${build.progress}[%3d%%]:${clear} %scompiling.$(mode) %s", progress, _g.ccache and "ccache " or "", sourcefile)
    end
...
function _build_files_for_single(target, sourcebatch, jobs)
    ...
    for index, sourcefile in ipairs(sourcefiles) do
        ...
        -- trace progress info
        if verbose then
            cprint("${build.progress}[%3d%%]:${clear} ${dim}%scompiling.$(mode) %s", progress, _g.ccache and "ccache " or "", sourcefile)
        else
            cprint("${build.progress}[%3d%%]:${clear} %scompiling.$(mode) %s", progress, _g.ccache and "ccache " or "", sourcefile)
        end
    end
